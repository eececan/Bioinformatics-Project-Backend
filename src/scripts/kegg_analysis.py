...
# This line is correct IF you run the script from the project's root directory
# (i.e., from 'your_project_directory/', not from inside 'src/')
PROGRESS_FILE = "data/kegg/kegg_analysis_progress.txt" # File to store the index of the last processed gene
API_DELAY_SECONDS = 0.5 # Delay between KEGG API calls to be polite
    ...
    if os.path.exists(PROGRESS_FILE):
        try:
            with open(PROGRESS_FILE, "r") as f:
                ...
                if content:
                    return int(content)
                return -1 # File exists but empty
        except ValueError:
...
def save_last_processed_index(index):
    ...
    try:
        with open(PROGRESS_FILE, "w") as f:
            f.write(str(index))
        # print(f"Progress saved: Processed up to index {index}.") # Can be verbose
    except Exception as e:
        print(f"Error saving progress to '{PROGRESS_FILE}': {e}")

# --- KEGG API Functions (with delays and improved error handling) ---
    ...
    try:
        ...
        for line in response.text.strip().split("\n"):
            if line.startswith(f"{species_code}:"):
                # print(f"  Found KEGG ID line: {line}")
...
def get_pathways_for_gene(entrez_id):
    ...
    try:
        ...
        for line in response.text.strip().split("\n"):
            ...
            if len(parts) < 2:
                continue
            # Ensure the pathway ID is correctly prefixed (e.g., "path:mmu00010")
            if parts[1].startswith("path:"):
                pathway_id = parts[1].split(":")[1] # Get "mmu00010"
                results.append(pathway_id)
        # print(f"  Pathways found for {entrez_id}: {results}")
        return results
    except requests.exceptions.RequestException as e:
...
def get_pathway_name(pathway_id):
    ...
    full_pathway_id_for_kegg = f"path:{pathway_id}" # KEGG API expects path:mmuXXXXX or just mmuXXXXX for get
    url = f"{KEGG_BASE_URL}/get/{pathway_id}" # Using just pathway_id works for /get/
    ...
    try:
        ...
        for line in response.text.strip().split("\n"):
            if line.startswith("NAME"):
                # print(f"  Pathway Name line: {line}")
                return line.split("NAME")[1].strip().split(" - ")[0] # Often "Pathway Name - Species"
...
def fetch_genes_from_db():
    ...
    with db_connect() as session:
        result = session.run("MATCH (t:Target) WHERE t.name IS NOT NULL RETURN t.name AS symbol ORDER BY t.name") # Added ORDER BY for consistency
...
def main():
    ...
    # Create a single session for DB checks to avoid overhead if many skips occur
    with db_connect() as db_session_for_checks:
        for current_idx in range(total_genes):
            ...
            if current_idx < start_index:
                # This gene was processed in a previous run according to the progress file
                # print(f"Skipping {gene_symbol} (index {current_idx}) based on progress file.")
                genes_skipped_this_run +=1
                continue

            print(f"\n--- Processing Gene {current_idx + 1}/{total_genes}: {gene_symbol} (Index: {current_idx}) ---")

            # Belt-and-suspenders: Check if DB already has this gene processed for KEGG
            # This is useful if progress file was lost or running on a partially populated DB
            if is_gene_kegg_processed_in_db(db_session_for_checks, gene_symbol):
                ...
                save_last_processed_index(current_idx) # Update progress to include this skip
                genes_skipped_this_run +=1
                continue
            ...
            if not entrez_id:
                ...
                # We still save progress, as we've "processed" (attempted) this gene.
                # If you want to retry genes with no Entrez ID on next run, do NOT save progress here.
                # For now, assume if not found, it's done for this script.
            ...
            # Successfully processed this gene (or attempted and found no pathways)
            save_last_processed_index(current_idx)
            genes_processed_this_run +=1
